...
export const createPoll = async (
  ...
  // Generate a unique operation key
  const opKey = generateOperationKey('createPoll', [
    await accountSigner.getAddress(),
    recipientAddress,
    pollForm,
    metadataUri,
  ...
  return executeOperation(
    ...
    async (client, builder) => {
      try {
        ...
        const dpollsContract = new ethers.Contract(
        ...
        // Prepare the mint function call data
        const callData = dpollsContract.interface.encodeFunctionData('createPoll', [
          pollForm.subject,
          pollForm.options,
        ...
        const userOp = await builder.execute(CONTRACT_ADDRESSES.dpollsContract, 0, callData);
        
        // Send the user operation with retry capability
        if (API_OPTIMIZATION.debugLogs) console.log(`Sending dpoll creation operation to paymaster`);
        const res = await client.sendUserOperation(userOp);
        ...
        console.error("Error in create poll operation:", error);
        throw error;
      }
    },
    paymentType,
